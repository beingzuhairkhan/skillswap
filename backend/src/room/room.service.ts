...
export class RoomService {
  ...
  async decodeMeetLink(meetLink: string, userId: string): Promise<any> {
    try {
      if (!userId || !meetLink)
        throw new NotFoundException('User or meet link not found');
      ...
      throw new InternalServerErrorException(
        error.message || 'Failed to decode meet link',
      );
    }
  }
  ...
  async convertAudioToText(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        error.message || 'failed to covert audio into text',
      );
    }
  }
    ...
    try {
      ...
      const feedback = new this.feedbackModel({
        ...
        currentUserId: data.currentUserId
          ? new Types.ObjectId(data.currentUserId)
          : new Types.ObjectId(requesterId),
        otherUserId: data.otherUserId
          ? new Types.ObjectId(data.otherUserId)
